...
export default function SignUpPage() {
    const { isLoading, handleSignUp, signUpForm } = useAuth()
    const {
        register,
        handleSubmit,
        formState: { errors }
    ...
    return (
        <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-black text-[#EDEDED] p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="flex w-full max-w-md flex-col gap-8 relative z-10">
                ...
                <SignUpForm handleSubmit={handleSubmit} handleSignUp={handleSignUp} register={register} errors={errors} isLoading={isLoading} />
            </div>
        </div>
    )
}
